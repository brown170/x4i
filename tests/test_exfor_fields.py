...
#
#   0.     improve legibility & get rid of depricated 'U' mode (David Brown <dbrown@bnl.gov>, 2018-12-05T20:24:28)
#   1.     make sure output is sorted by pointer in ALL cases & adjust unit test accordingly (David Brown <dbrown@bnl.gov>, 2018-12-05T20:03:27)
#   2.     split up the tests (David Brown <dbrown@bnl.gov>, 2018-12-05T19:57:31)
#   3.     dealing with map() and open() issues (David Brown <dbrown@bnl.gov>, 2018-12-05T19:49:24)
#   4.     some {}.keys() calls that needed to be converted to lists (David Brown <dbrown@bnl.gov>, 2018-12-05T19:15:51)
...
#   7.     Python3.X compatibility & PEP-8 compliance (David Brown <dbrown@bnl.gov>, 2018-12-04T15:18:59)
#   8.     only one bug left (David Brown <dbrown@bnl.gov>, 2013-06-26T22:44:24)
#   9.     fix reference parsing & fix reaction parsing when have isomer projectile, isomer math at same time (David Brown <dbrown@bnl.gov>, 2013-06-26T22:32:42)
#   10.     fooled by complicated product lists no more (David Brown <dbrown@bnl.gov>, 2013-06-26T20:30:07)
#   11.     deal with corner case of X4Reaction that is really a isomer math case that slipped past the checks (David Brown <dbrown@bnl.gov>, 2013-06-26T18:47:07)
#   12.     fix bug with reaction list when handling isomer math;  add unit tests, lots of unit tests (David Brown <dbrown@bnl.gov>, 2013-06-26T17:51:53)
#   13.     parsing even really complicated monitors now work and all are printed when you print a data set (David Brown <dbrown@bnl.gov>, 2013-06-18T14:40:59)
#   14.     trying to get monitors working (David Brown <dbrown@bnl.gov>, 2013-06-18T13:26:10)
#   15.     correct parsing of monitors to properly distinguish between multiple monitors and those with associated column headings in DATA section (David Brown <dbrown@bnl.gov>, 2013-01-11T18:58:32)
#   16.     monitors now parsed and packed into a data structure (David Brown <dbrown@bnl.gov>, 2013-01-10T15:55:39)
#   17.     beginning to code up monitor unit tests (David Brown <dbrown@bnl.gov>, 2013-01-09T19:37:43)
#   18.     add optional call to XML test runner (David Brown <dbrown@bnl.gov>, 2012-01-23T19:09:00)
#
################################################################################
...
from x4i import exfor_field

test_entry = (''.join(open(os.path.dirname(__file__) + os.sep + 'E0783.x4', mode='r').readlines())).replace('\r', '')
...
test_entry_2 = (''.join(open(os.path.dirname(__file__) + os.sep + '12898.x4', mode='r').readlines())).replace('\r', '')
...
class TestX4ReactionField(unittest.TestCase):
    ...
    def test_str(self):
        self.assertEqual(str(self.field),
                         '(( Resonance integral for 64Zn(n,gamma)65Zn )/( Cross section for 64Zn(n,gamma)65Zn ))')
    ...
    def test_oh_god_no_2(self):
        ...
        testField = exfor_field.X4ReactionField(testFieldString.split('\n'))
        self.assertEqual(str(testField),
                         '[1] Cross section for 178m2Hf(n,gamma)179m0Hf; '
                         '[2] Resonance integral for 178m2Hf(n,gamma)179m0Hf; '
                         '[3] (( Cross section ratio for 178m2Hf(n,gamma)179m2Hf )/( Cross section ratio for 178m2Hf(n,gamma)179m0Hf )); '
                         '[4] Cross section for 178m2Hf(n,gamma)179m2Hf; '
                         '[5] Resonance integral for 178m2Hf(n,gamma)179m2Hf'
                         )

    def test_oh_god_no_3(self):
...
class TestX4ReactionFieldPointer(unittest.TestCase):
    ...
    def test_str(self):
        self.assertEqual(str(self.field),
                         '[1] (( Resonance integral for 64Zn(n,gamma)65Zn )/( Cross section for 64Zn(n,gamma)65Zn )); '
                         '[2] Resonance integral for 64Zn(n,gamma)65Zn, Reduced resonance integral and ratio to thermal absorption cross section')
...
class TestX4TitleField(unittest.TestCase):
    ...
    def test_oneline(self):
        ...
        self.assertEqual(repr(exfor_field.X4TitleField(self.short_example.split('\n'))), self.short_example)
    ...
    def test_multiline(self):
        ...
        self.assertEqual(repr(exfor_field.X4TitleField(self.long_example.split('\n'))), self.long_example)
...
class TestX4MonitorField(unittest.TestCase):
    ...
    def test_oneline_pointer(self):
        ...
        self.assertEqual(str(X.reactions), "{'1': [((79-AU-197(N,2N)79-AU-196,SIG), '', None)]}")
    ...
    def test_multiline(self):
        ...
        self.assertEqual(X.reactions[' '][0][1],
    ...
    def test_three_monitors(self):
        ...
        self.assertEqual(str(X.reactions),
                         "{' ': [((22-TI-0(D,X+23-V-48),SIG), '21 MeV', None), ((13-AL-27(D,X+11-NA-22),SIG), '40 MeV', None), ((13-AL-27(D,X+11-NA-24),SIG), '40 MeV', None)]}")
        self.assertEqual(list(X.reactions.keys()), [' '])
    ...
    def test_combo(self):
        ...
        self.assertEqual(str(X), "[((( (29-CU-63(A,N+P)30-ZN-65,TTY) )+( (29-CU-63(A,2N)31-GA-65,TTY) )), '', None)]")
    ...
    def test_kitchen_sink(self):
        ...
        self.assertEqual(str(X),
                         "[((( (95-AM-241(N,F+42-MO-99),CUM,FY,SPA) )+( (95-AM-241(N,F+43-TC-99-M),CUM,FY,SPA) )), 'USED FOR NORMALIZATION OF THE LIGHT-MASS PEAK, I.E. A = 91 TO 105.', 'MONIT1'), ((95-AM-241(N,F+56-BA-139),CUM,FY,SPA), 'USED FOR NORMALIZATION OF THE HEAVY-MASS PEAK, I.E. A = 131 TO 147.', 'MONIT2')]")
...
class TestX4AuthorField(unittest.TestCase):
    ...
    def test_oneline(self):
        self.assertEqual(repr(exfor_field.X4AuthorField(self.short_example.split('\n'))), self.short_example)
        self.assertEqual(str(exfor_field.X4AuthorField(self.short_example.split('\n'))),
                         'D.L.Smith, J.W.Meadows, I.Kanno')
        self.assertEqual(exfor_field.X4AuthorField(self.short_example.split('\n')).author_family_names,
                         ['Smith', 'Meadows', 'Kanno'])

    def test_multiline(self):
        self.assertEqual(repr(exfor_field.X4AuthorField(self.long_example.split('\n'))), self.long_example)
        self.assertEqual(exfor_field.X4AuthorField(self.long_example.split('\n')).author_family_names,
                         ['Hatanaka', 'Matsuoka', 'Sakai', 'Saito', 'Tamura', 'Hosono', 'Kondo', 'Imai', 'Shimizu',
                          'Nishimura'])
    ...
    def test_really_long(self):
        self.assertEqual(repr(exfor_field.X4AuthorField(self.really_long_example.split('\n'))),
                         self.really_long_example)
        self.assertEqual(exfor_field.X4AuthorField(self.really_long_example.split('\n')).author_family_names,
                         ['Budzanowski', 'Chatterjee', 'Gebel', 'Hawranek', 'Jahn', 'Jha', 'Kilian', 'Kliczewski',
                          'Kirillov', 'Kirillov', 'Kolev', 'Kravcikova', 'Lesiak', 'Lieb', 'Machner', 'Magiera',
                          'Maier', 'Martinska', 'Nedev', 'Piskunov', 'Prasuhn', 'Protic', 'Ritman', 'Von Rossen', 'Roy',
                          'Sitnik', 'Siudak', 'Tsenov', 'Urban', 'Vankova', 'Wilkin'])
        self.assertEqual(str(exfor_field.X4AuthorField(self.really_long_example.split('\n'))),
...
class TestX4InstituteField(unittest.TestCase):
    ...
    def test_oneline(self):
        self.assertEqual(repr(exfor_field.X4InstituteField(self.short_example.split('\n'))), self.short_example)
        self.assertEqual(str(exfor_field.X4InstituteField(self.short_example.split('\n'))),
...
class TestX4ReferenceField(unittest.TestCase):
    ...
    def test_oneline(self):
        self.assertEqual(repr(exfor_field.X4ReferenceField(self.short_example.split('\n'))), self.short_example)
    ...
    def test_doi(self):
        self.assertEqual(repr(exfor_field.X4ReferenceField(self.doi.split('\n'))), self.doi)
        self.assertEqual(str(exfor_field.X4ReferenceField(self.doi.split('\n'))),
                         'Physical Review, Part C, Nuclear Physics 72, 034302 (2005)')

    def test_multiline(self):
        self.assertEqual(repr(exfor_field.X4ReferenceField(self.long_example.split('\n'))), self.long_example)
    ...
    def test_reallylong(self):
        self.assertEqual(repr(exfor_field.X4ReferenceField(self.really_long_example.split('\n'))),
                         self.really_long_example)
    ...
    def test_tautology(self):
        self.assertEqual(repr(exfor_field.X4ReferenceField(self.tautology_example.split('\n'))), self.tautology_example)
    ...
    def test_another_tautology(self):
        self.assertEqual(repr(exfor_field.X4ReferenceField(self.another_tautology_example.split('\n'))),
    ...
    def test_tautology_string(self):
        self.assertEqual(repr(exfor_field.X4ReferenceField(self.tautology_string.split('\n'))), self.tautology_string)
