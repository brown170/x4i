...
#
#   0.     Fix spelling: grammar is spelled "grammar" not "grammer" (David Brown <dbrown@bnl.gov>, 2018-12-05T16:41:09)
#
################################################################################  

# module exfor_grammars.py
"""
exfor_grammars module - Collection of pyparsing grammars for parsing Exfor objects
"""
__version__ = "0.0.1"
...
mathop_noshow = dashorplus ^ Literal('//').suppress() ^ Literal('*').suppress() ^ Literal('/').suppress()

altmultop = mult | doubleslash | div | equals
altaddop = plus | minus

# ------------------------------------------------------
# Define the grammar of an Exfor Reaction Field ...
# ------------------------------------------------------
...
x4basicparticle = Optional(Literal('X')) + MatchFirst([Literal("%s"%x) for x in get_exfor_dict("Particles").keys()])
...
x4chemical_compound = MatchFirst([Literal("%s"%x) for x in get_exfor_dict("Chemical compounds").keys()])
# eval('^'.join(['Literal( \"' + x + '\" )' for x in get_exfor_dict("Chemical compounds").keys()]))
...
x4process = x4target + lpar + x4projectile + comma + x4products + rpar + Optional(x4residual)
x4reaction_descriptor = Word(alphanums + '/*+-') ^ (lpar + Word(alphanums + '/*+-') + rpar)
x4reaction = lpar + delimitedList(Group(x4process) ^ ZeroOrMore(x4reaction_descriptor)) + rpar
...
# x4reactionfield = OneOrMore(x4compound_expression)+restOfLine #should only be one reaction in a field


x4isomermath = Word(nums) + dash + x4element + dash + Word(nums) + ZeroOrMore(mathop + x4isomer_modifier)
...
x4isomerprocess = x4target + lpar + x4projectile + comma + x4isomerproducts + rpar + Optional(x4isomerresidual)
x4isomerreaction = lpar + delimitedList(Group(x4isomerprocess) ^ ZeroOrMore(x4reaction_descriptor)) + rpar
x4isomermathreactionfield = x4isomerreaction + restOfLine

# ------------------------------------------------------
# Define the grammar of an Exfor Text Field
# ------------------------------------------------------
x4word = Word(alphanums + ",.;+-*/'")
x4minicode = lpar + Word(alphanums + "-/") + rpar
x4phrase = OneOrMore(x4word | x4minicode)
x4code = (lpar + x4phrase + rpar) ^ (lpar + lpar + x4phrase + rpar + equals + lpar + x4phrase + rpar + rpar)
x4codefield = Group(x4code) + ZeroOrMore(x4word)
x4textfield = OneOrMore(Group(x4codefield))

#x4institute = Word(alphanums + " ", exact=7)  
x4institute = MatchFirst([Keyword("%s"%x) for x in get_exfor_dict("Institutes").keys()])
#print(x4institute)
