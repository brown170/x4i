...
#   2.     resolve universal newline change with wrapped open() function (David Brown <dbrown@bnl.gov>, 2018-12-05T20:54:04)
#   3.     improve legibility & get rid of depricated 'U' mode (David Brown <dbrown@bnl.gov>, 2018-12-05T20:24:28)
#   4.     dealing with map() and open() issues (David Brown <dbrown@bnl.gov>, 2018-12-05T19:49:24)
#   5.     undo attempts to update universal newline support flags (David Brown <dbrown@bnl.gov>, 2018-12-05T17:09:40)
#   6.     csv file managemnet for py3 (David Brown <dbrown@bnl.gov>, 2018-12-05T02:01:43)
#   7.     no xmlrunner for now (David Brown <dbrown@bnl.gov>, 2018-12-04T22:06:10)
#   8.     from __future__ imports missing (David Brown <dbrown@bnl.gov>, 2018-12-04T18:36:38)
#   9.     Python3.X compatibility & PEP-8 compliance (David Brown <dbrown@bnl.gov>, 2018-12-04T15:18:59)
#   10.     deal with csv files with almost (but not exactly) same table entries (David Brown <dbrown@bnl.gov>, 2013-10-10T15:12:23)
#   11.     add optional call to XML test runner (David Brown <dbrown@bnl.gov>, 2012-01-23T19:09:00)
#
################################################################################
...
class TestX4Entry(TestCaseWithTableTests):
    ...
    def test_meta(self):
        ...
        self.assertEqual(exfor_entry.X4Entry(self.entry).meta().citation(),
    ...
    def test_getDataSets(self):
        ...
        self.assertTablesAlmostEqual(str(ds[('E0783', 'E0783002', ' ')]), answer)
    ...
    def test_getDataSets_2_cross_section_translation(self):
        ...
        with open_for_reading_universal_newline_flag('junk5.csv') as csvfile:
            ...
            for i in range(2, len(left)):
                leftRow = [float(x) for x in left[i].split(',')]
                rightRow = [float(x) for x in right[i].split(',')]
...
class TestX4DataSection(unittest.TestCase):
    ...
    def test_simple_common(self):
        ...
        self.assertEqual(str(x4ds),
                         'COMMON               1          1\nEN         \nMEV        \n56.0       \nENDCOMMON            3')
        self.assertEqual(len(x4ds), 1)
        self.assertEqual(x4ds.pointers, {})
        x4ds.csv('junk1.csv')
        with open_for_reading_universal_newline_flag('junk1.csv') as csvfile:
            self.assertEqual(csvfile.readlines(), ['EN\n', 'MEV\n', '56.0\n'])
        os.remove('junk1.csv')
    ...
    def test_complicated_data(self):
        ...
        x4ds.csv('junk2.csv')
        with open_for_reading_universal_newline_flag('junk2.csv') as csvfile:
            self.assertEqual(csvfile.readlines(), [
