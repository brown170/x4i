...
#
#   0.     no xmlrunner for now (David Brown <dbrown@bnl.gov>, 2018-12-04T22:06:10)
#   1.     from __future__ imports missing (David Brown <dbrown@bnl.gov>, 2018-12-04T18:36:38)
#   2.     Python3.X compatibility & PEP-8 compliance (David Brown <dbrown@bnl.gov>, 2018-12-04T15:18:59)
#   3.     add a unit test for reactions to the ground state (David Brown <dbrown@bnl.gov>, 2015-06-18T17:41:57)
#   4.     fix harmless repr() bug in chemical compounds (David Brown <dbrown@bnl.gov>, 2013-07-01T15:20:19)
#   5.     isomer math parsing works in unit tests, now to get it to work in a field (David Brown <dbrown@bnl.gov>, 2013-06-26T17:15:40)
#   6.     add optional call to XML test runner (David Brown <dbrown@bnl.gov>, 2012-01-23T19:09:00)
#
################################################################################
...
from x4i import exfor_reactions
from x4i import exfor_exceptions
from x4i import exfor_particle
...
class TestX4Nucleus(unittest.TestCase):
    ...
    def test_isomers(self):
        ...
        za = exfor_reactions.X4Isomer("11-NA-24-G")
        self.assertEqual(str(za), '24m0Na')
        self.assertEqual(repr(za), '11-NA-24-M0')
        self.assertEqual(za.asList(), ['11', 'NA', '24', '-', 'G'])
        self.assertEqual(za.endlZAStyle(), 'za011024g')
        self.assertEqual(za.exforStyle(), '11-NA-24-M0')
        self.assertEqual(za.prettyStyle(), '24m0Na')

    def test_gs_isomer_sum(self):
        self.assertRaises(exfor_exceptions.IsomerMathParsingError, lambda x: exfor_reactions.X4Isomer(x),
                          "94-Pu-240-G+M")
        self.assertRaises(exfor_exceptions.IsomerMathParsingError, lambda x: exfor_reactions.X4Isomer(x),
                          "94-Pu-240-M1+M2")
...
#    def test_4n( self ):
#        self.assertEqual( str( exfor_particle.X4Particle( exfor_grammars.x4particle.parseString( '4N' ) ) ), '4n' )
#    def test_3he3( self ):
#        self.assertEqual( str( exfor_particle.X4Particle( exfor_grammars.x4particle.parseString( ) ) ),  )

class TestX4ChemicalCompound(unittest.TestCase):
    def test(self):
        self.assertEqual(str(exfor_particle.X4ChemicalCompound("19-K-CMP")), 'Potassium compound')
        self.assertEqual(exfor_particle.X4ChemicalCompound("19-K-CMP").Z, 19)


class TextX4Process(unittest.TestCase):
    def test_n2n(self):
        self.assertEqual(str(exfor_reactions.X4Process("94-PU-240(N,2n)")), '240Pu(n,2n)239Pu')

    def test_nnprime(self):
        self.assertEqual(str(exfor_reactions.X4Process("94-PU-240(N,N)94-PU-240")), '240Pu(n,n)240Pu')

    def test_ntot(self):
        self.assertEqual(str(exfor_reactions.X4Process("94-PU-240(N,TOT)")), '240Pu(n,Total)')

    def test_nf(self):
        self.assertEqual(str(exfor_reactions.X4Process("94-PU-240(N,F)")), '240Pu(n,Fission)')

    def test_spontaneous_fission(self):
        self.assertEqual(str(exfor_reactions.X4Process("94-PU-240(0,F)")), '240Pu(None,Fission)')

    def test_2p4n(self):
        self.assertEqual(str(exfor_reactions.X4Process("94-PU-240(P,2p+4n)")), '240Pu(p,2p+4n)235Np')

    def test_heavy_ion(self):
        self.assertEqual(str(exfor_reactions.X4Process("94-PU-240(94-PU-240,X)")), '240Pu(240Pu,Unspecified)')

    def test_spallation(self):
        self.assertEqual(str(exfor_reactions.X4Process("94-PU-240(P,A+6-C-14+4N)")), '240Pu(p,alpha+14C+4n)219Fr')

    def test_kitchen_sink(self):
        self.assertEqual(str(exfor_reactions.X4Process("94-PU-240(P,A+6-C-14+4N+X)")),
                         '240Pu(p,alpha+14C+4n+Unspecified)')

    def test_isomer_target(self):
        self.assertEqual(str(exfor_reactions.X4Process("94-PU-240-M(N,2n)")), '240mPu(n,2n)239Pu')

    def test_simple_but_important(self):
        self.assertEqual(str(exfor_reactions.X4Process("94-PU-239(N,2N)94-PU-238")), '239Pu(n,2n)238Pu')

    def test_chemical_target(self):
        # print
        # print exfor_grammars.x4chemical_compound
        # print
        # print exfor_grammars.x4target
        # print
        # print exfor_grammars.Group(exfor_grammars.x4nucleus)
        # print
        # print exfor_grammars.x4target.parseString( "19-K-CMP" )
        # print
        self.assertEqual(str(exfor_reactions.X4Process("19-K-CMP(N,TOT)")), 'Potassium compound(n,Total)')
        self.assertEqual(repr(exfor_reactions.X4Process("19-K-CMP(N,TOT)")), "19-K-CMP(N,TOT)")


class TestX4Reaction(unittest.TestCase):
    def test_simple_cs(self):
        self.assertEqual(str(exfor_reactions.X4Reaction("(94-PU-239(N,F),,SIG)")), 'Cross section for 239Pu(n,Fission)')

    def test_hard_cs(self):
        self.assertEqual(str(exfor_reactions.X4Reaction("(94-PU-239(P,A+6-C-14+4N),,SIG)")),
                         'Cross section for 239Pu(p,alpha+14C+4n)218Fr')
...
class TestX4ReactionCombination(unittest.TestCase):
    def test_sum(self):
        self.assertEqual(
            str(exfor_reactions.X4ReactionCombination("((92-U-235(N,G)92-U-236,,SIG)+(92-U-235(N,F),,SIG))")),
            '(( Cross section for 235U(n,gamma)236U )+( Cross section for 235U(n,Fission) ))')

    def test_ratio(self):
        self.assertEqual(str(exfor_reactions.X4ReactionCombination("((92-U-234(N,F),,SIG)/(92-U-235(N,F),,SIG))")),
    ...
    def test_sum_with_isomer(self):
        self.assertEqual(str(exfor_reactions.X4ReactionCombination(
            "((79-AU-197(N,2N)79-AU-196-G,,SIG)+(79-AU-197(N,2N)79-AU-196-M1,,SIG))")),
                         '(( Cross section for 197Au(n,2n)196m0Au )+( Cross section for 197Au(n,2n)196mAu ))')

    def test_you_are_friggen_kidding(self):
        self.assertEqual(str(exfor_reactions.X4ReactionCombination(
            "((79-AU-197(N,2N)79-AU-196-M2,,SIG)/(79-AU-197(N,2N)79-AU-196-G+M1,,SIG))")),
