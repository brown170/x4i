...
#
#   0.     spelling error (David Brown <dbrown@bnl.gov>, 2019-02-27T03:10:54)
#   1.     eliminate more map() calls (David Brown <dbrown@bnl.gov>, 2018-12-05T19:06:07)
#   2.     another change from map() (David Brown <dbrown@bnl.gov>, 2018-12-05T01:24:15)
#   3.     fixing imports (David Brown <dbrown@bnl.gov>, 2018-12-04T21:09:44)
#   4.     Python3.X compatibility & PEP-8 compliance  (David Brown <dbrown@bnl.gov>, 2018-12-04T17:44:23)
#   5.     fix reference parsing & fix reaction parsing when have isomer projectile, isomer math at same time (David Brown <dbrown@bnl.gov>, 2013-06-26T22:32:42)
#   6.     fooled by complicated product lists no more (David Brown <dbrown@bnl.gov>, 2013-06-26T20:30:07)
#   7.     deal with corner case of X4Reaction that is really a isomer math case that slipped past the checks (David Brown <dbrown@bnl.gov>, 2013-06-26T18:47:07)
#   8.     fix bug with reaction list when handling isomer math & add unit tests, lots of unit tests (David Brown <dbrown@bnl.gov>, 2013-06-26T17:51:53)
#   9.     isomer math parsing works in unit tests, now to get it to work in a field (David Brown <dbrown@bnl.gov>, 2013-06-26T17:15:40)
#   10.     add X4MonitorField & add blank line between member functions for clarity (David Brown <dbrown@bnl.gov>, 2013-01-08T19:31:15)
#   11.     rework node & link generation (David Brown <dbrown@bnl.gov>, 2012-08-15T16:24:48)
#   12.     attempting to do isomer math (David Brown <dbrown@bnl.gov>, 2012-01-04T21:10:25)
#   13.     handle variable particles better (David Brown <dbrown@bnl.gov>, 2012-01-03T19:59:53)
#
################################################################################
...
class X4Process:
    ...
    def __init__(self, xl, IgnoreIsomerMath=False):
        ...
        self.products = [self.setProduct(x) for x in xl[2:]]
        self.setResidual()
        self.setProcessType()
    ...
    def __repr__(self):
        ...
        result += '+'.join([repr(x) for x in self.products]) + ')'
        if self.residual:
    ...
    def parse(self, x):
        try:
            ...
            raise ReactionParsingError('Can not parse process "' + x + '",\n    got error "' + str(err) + '"\n   ')
    ...
    def setProduct(self, i):
        """
        Figures out what the product of a reaction is, whether it is one or more particles or whether it
        is a catch-all (e.g. Nonelastic)
        """
    ...
    def setResidual(self):
        ...
        for channel in ['EL', 'Elastic', 'SCT', 'Scattering', 'THS', 'ThermalScattering', 'INL', 'Inelastic']:
        ...
        if Zin == Zout and Ain == Aout:
            ...
            try:
                self.residual = X4Nucleus([str(Zin - Zout), endl_ZSymbol(Zin - Zout), str(Ain - Aout)])
                ...
                if (Zin - Zout) == -1 and (Ain - Aout) == 0:
                    ...
                    raise ResidualNucleusError(
                        "Cannot compute residual, Delta Z = " + str(Zin - Zout) + ", Delta A = " + str(
                            Ain - Aout) + ", for reaction " + str(self.parse_results))
        else:
            self.residual = None
    ...
    def setProcessType(self):
        ...
        self.processType = '+'.join([str(swapit(x)) for x in self.products])
...
class X4Reaction(X4Process, X4Measurement):
    ...
    def parse(self, x):
        try:
            ...
            raise ReactionParsingError('Can not parse reaction "' + x + '",\n    got error "' + str(err) + '"\n   ')
...
class X4ReactionCombination(X4Measurement):
    """Parsed Exfor reaction combination.  This is what is measured when """

    def __init__(self, x):
        X4Measurement.__init__(self, None, None)
        if type(x) == str:
            x = self.parse(x)
        # Extract part of line contained in either a reaction or mathematical expression
        self.data = [self.getStrOrX4Reaction(xx) for xx in x]
    ...
    def parse(self, x):
        try:
            ...
            raise ReactionParsingError(
                'Can not parse compound reaction expression "' + x + '",\n    got error "' + str(err) + '"\n   ')
    ...
    def __str__(self):
        return self.print_gizmo(str)

    def __repr__(self):
        return self.print_gizmo(repr)
...
class X4ReactionIsomerCombination(X4ReactionCombination):
    ...
    def repack(self, x):
        ...
        for residPart in residList:
            if type(residPart) == list:  # we have a new residual, therefore a new reaction
                result.append('(' + targ + '(' + proj + ',' + proc + ')' + '-'.join(residPart) + ',' + quant + ')')
...
if __name__ == "__main__":
    ...
    for i in ["94-PU-240", "94-Pu-240", "94-PU-240-", "94-PU-240-M", "94-Pu-240-M2", "94-Pu-240-G+M"]:
        il = x4nucleus.parseString(i)
        za = X4Nucleus(il)
        print(i.ljust(20), str(il).ljust(40), repr(za).ljust(20), za.endlZAStyle().ljust(20), za.exforStyle().ljust(20))
    # ---------------------------------
    print()
    ...
    for i in ["(N,2n)", "(N,N)94-PU-240", "(N,TOT)", "(N,F)", "(P,2p+4n)", "(94-PU-240,X)", '(0,F)', '(P,A+6-C-14+4N)',
              '(P,A+6-C-14+4N+X)']:  # has trouble with isomer targets
        ii = "94-PU-240" + i
    ...
    for i in ["(94-PU-239(N,F),,SIG)", "(92-U-238-M(0,F),TER,AKE,LCP)", "(94-PU-239(N,ABS),,SIG,,MXW)",
              "(94-PU-239(P,A+6-C-14+4N),,SIG)", "(94-PU-240(N,0),,EN)"]:
    ...
    infile = open('Testing/test_rxn_strings.txt', mode='r')
    ...
    for line in lines:
        if line[0:8] == 'REACTION' or line[10] != ' ':
            ...
            if line[10] != ' ': pointers[len(rxnfieldlist) - 1] = line[10]
        else:
            lastfield += ' ' + line[11:66].strip()
    rxnfieldlist.append(lastfield)
    rxnfieldlist.pop(0)
